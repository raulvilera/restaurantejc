        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-border-dark">
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-slate-500">Carregando pedidos...</td>
                </tr>
              ) : orders.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-slate-500">Nenhum pedido encontrado.</td>
                </tr>
              ) : orders.map((order) => (
                <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-background-dark/20 transition-colors group">
                  <td className="px-6 py-5 whitespace-nowrap">
                    <span className="text-sm font-bold text-primary">#{order.id.substring(0, 4)}</span>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-1">{getTimeAgo(order.created_at)}</p>
                  </td>
                  <td className="px-6 py-5 whitespace-nowrap">
                    <div className="flex items-center gap-3">
                      <div className="size-8 rounded-full bg-slate-100 dark:bg-background-dark flex items-center justify-center text-[10px] font-bold text-slate-500">
                        {getInitials(order.customer_name || 'Anônimo')}
                      </div>
                      <span className="text-sm font-semibold">{order.customer_name || 'Anônimo'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-1 max-w-xs">{order.items_content || 'Itens não especificados'}</p>
                  </td>
                  <td className="px-6 py-5 whitespace-nowrap text-sm font-bold">{formatCurrency(order.total_amount || 0)}</td>
                  <td className="px-6 py-5 whitespace-nowrap">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border ${statusStyles[order.status as OrderStatus] || statusStyles[OrderStatus.PENDING]}`}>
                      {order.status}
                    </span>
                  </td>
